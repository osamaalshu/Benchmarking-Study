...
class MAUNetErrorAware(nn.Module):
    """
    Error-Aware MAUNet based on ResNet backbone with triple decoder paths
    for classification, regression (distance transform), and centroid prediction
    
    Enhanced with composite loss and improved post-processing for better
    instance detection and reduced false negatives.
    """
...
def create_maunet_error_aware_model(
    num_classes: int = 3,
    input_size: int = 256,
    in_channels: int = 3,
    ...
    Returns:
        MAUNetErrorAware model instance
    """
    model = MAUNetErrorAware(
...
class MAUNetErrorAwareEnsemble(nn.Module):
    """Ensemble wrapper that averages logits from multiple Error-Aware MAUNet backbones.

    This mirrors the "complete model" reported by the saltfish team, which
    ensembles ResNet50 and Wide-ResNet50 variants, enhanced with centroid prediction.
    """

    def __init__(
        self,
        models_list: List[MAUNetErrorAware],
        average: bool = True,
    ) -> None:
        super().__init__()
        if not models_list:
            raise ValueError("models_list must contain at least one MAUNetErrorAware instance")
...
def create_maunet_error_aware_ensemble_model(
    num_classes: int = 3,
    input_size: int = 256,
    in_channels: int = 3,
    ...
    return MAUNetErrorAwareEnsemble(models_list=members, average=average)