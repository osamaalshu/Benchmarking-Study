...
This benchmarking study was conducted on the **NeurIPS 2022 Cell Segmentation Challenge dataset** (neurips22-cellseg.grand-challenge.org). This is a comprehensive multi-modal microscopy image dataset designed to benchmark cell segmentation algorithms across diverse imaging conditions. The dataset is designed to test how well different cell segmentation models handle a range of microscopy images—different cell sources (tissue and cultured), various stains (like Jenner-Giemsa, fluorescent), imaging types (brightfield, fluorescent, phase-contrast, DIC), and all sorts of cell shapes. Because of all this variety, it's considered one of the tougher benchmarks out there.

For this study, we adopted a **fully supervised approach** using 1,000 labeled images from the main dataset, rather than following the challenge's weakly supervised setup. The data was split into 900 images for training (with 100 set aside for validation), and we used 101 extra images from the challenge's tuning dataset as a separate test set. We chose to perform **instance segmentation** on the data since it is more challenging than semantic segmentation.

For evaluation, we used standard metrics for instance segmentation (F1, Dice, Precision/Recall) at different IoU thresholds (0.5, 0.7, and 0.9), and analyzed performance across different cell counts. Using the tuning set as an independent test set ensures unbiased evaluation and comprehensive coverage of all the different types of data in the challenge.
